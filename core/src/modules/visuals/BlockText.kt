 ...
 */
class BlockText(var text: String, size: Int, colour: Color, var fontPath: String, w: lv = GetLcs.byLcs(1f), h: lv = GetLcs.byLcs(1f), var align: Int = 1, var padding: lv = GetLcs.byLcs(0f), var keepWords: Boolean = false, visualSize: VisualSize = VisualSize.STATIC) : OmniVisual(w, h, visualSize = visualSize) {
    var initSize = size
    ...
    override fun copy(): OmniVisual {
        return BlockText(text, initSize, font.color, fontPath, width, height, align, padding, keepWords, visualSize)
    }
