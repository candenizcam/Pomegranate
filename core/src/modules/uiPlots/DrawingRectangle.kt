...
data class DrawingRectangle(val screenCanvas: Rectangle, val punWidth: Float, val punHeight: Float, val canvasRatio: Rectangle){
    ...
    var u1 = (screenCanvas.left-screenSegment.left).coerceAtLeast(0f)/screenSegment.width // corresponds to u values of Sprite
    var u2 = 1 + (screenCanvas.right-screenSegment.right).coerceAtMost(0f)/screenSegment.width // crops a contained texture
    var v1 = - (screenCanvas.top-screenSegment.top).coerceAtMost(0f)/screenSegment.height // u are width, h are height
    var v2 = 1 - (screenCanvas.bottom-screenSegment.bottom).coerceAtLeast(0f)/screenSegment.height // and of course h are inverse coordinated but thats handled there
